...
grep -qxF 'GH issue list --state open --label needs-review --limit 300 --json number,title,updatedAt,parent,blockedBy' "$GH_LOG" \
  || fail "needs-review listing argv wrong: $(cat "$GH_LOG")"
grep -qxF 'GH issue list --state open --label blocked --limit 300 --json number,title,updatedAt,parent,blockedBy' "$GH_LOG" \

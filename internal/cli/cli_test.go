...
func TestEachCommandHelpRenders(t *testing.T) {
	for _, cmd := range []string{"version", "work", "status", "clean", "finish", "setup", "completion", "doctor", "update"} {
...
func TestRootHelpListsCommands(t *testing.T) {
	...
	for _, cmd := range []string{"work", "status", "clean", "finish", "setup", "completion", "doctor", "update", "version"} {
